...
module.exports = {
  ...
  calculate: function (s) {
    if (s.lookback[s.renko_options.min_periods]) {
      var prclose = Math.round(s.lookback[s.renko_options.min_periods].close)
      var propen = Math.round(s.lookback[s.renko_options.min_periods].close)
    }
  },
  ...
  onPeriod: function (s) {
    if (s.lookback[s.renko_options.min_periods]) {
      ...
      // Sources
      prclose = Math.round(s.lookback[s.renko_options.min_periods].close)
      propen = Math.round(s.lookback[s.renko_options.min_periods].close)
      for(var i = 0; i<s.renko_options.min_periods;i++)
      {
...
function renko_close(s, close, prclose, propen){
  var type = s.renko_options.bricksize*2	
  if (close > (prclose + type)) 
  {
    if (prclose > propen)
    {	
      return (prclose + s.renko_options.bricksize) 
    }
    else 
    {
      return (prclose + type)
