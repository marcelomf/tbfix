...
module.exports = {
  ...
  calculate: function (s) {
    ...
    bollinger(s, 'bollinger', s.bollinger_options.bollinger_size)
  },
  ...
  onPeriod: function (s) {
    if (s.period.bollinger) {
      if (s.period.bollinger.upperBound && s.period.bollinger.lowerBound) {
        ...
        if (s.period.close > (upperBound / 100) * (100 - s.bollinger_options.bollinger_upper_bound_pct)) {
          s.signal = 'sell'
	        s.period.bollinger_trend = 'sell' 
        } else if (s.period.close < (lowerBound / 100) * (100 + s.bollinger_options.bollinger_lower_bound_pct)) {
	        s.period.bollinger_trend = 'buy' 
          s.signal = 'buy'
        } else {
          s.signal = null // hold
        }
