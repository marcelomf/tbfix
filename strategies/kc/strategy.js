...
module.exports = {
  ...
  calculate: function (s) {
    ...
    kc(s, 'kc_idx', s.kc_options.kc_size)
  },

  onPeriod: function (s) {
    if (s.period.kc_idx) {
      if (s.period.kc_idx.upper && s.period.kc_idx.lower) {
        let upperChannel = s.period.kc_idx.upper[s.period.kc_idx.upper.length-1]
        let lowerChannel = s.period.kc_idx.lower[s.period.kc_idx.lower.length-1]
        if (s.period.close > (upperChannel / 100) * (100 - s.kc_options.kc_upper_channel_pct)) {  
          s.signal = 'sell'
        } else if (s.period.close < (lowerChannel / 100) * (100 + s.kc_options.kc_lower_channel_pct)) {
          s.signal = 'buy'
        } else {
          s.signal = null // hold
        }
