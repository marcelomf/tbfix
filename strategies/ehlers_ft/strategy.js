...
module.exports = {
  ...
  onPeriod: function(s) {
    ...
    if (!s.eft) {
      ...
      s.eft_max_elements = Math.max(s.ehlers_ft_options.pos_length, 3)
    }

    s.period.src = tv.src(s.ehlers_ft_options.src, s.period, s.lookback[0])

    let lbks = s.lookback.slice(0, s.ehlers_ft_options.length - 1).map(p => p.src),
    ...
    s.eft.pos.unshift(
      tv.iff(s.eft.fish[0] > tv.nz(s.eft.fish[1] * (1 + s.ehlers_ft_options.fish_pct_change / 100)), 1,
        tv.iff(s.eft.fish[0] < tv.nz(s.eft.fish[1] * (1 - s.ehlers_ft_options.fish_pct_change / 100)), -1, tv.nz(s.eft.pos[0], 0))))

    if (!s.in_preroll) {
      if (s.ehlers_ft_options.pos_length === 1) {
        if (s.eft.pos[0] === 1) {
          s.signal = 'buy'
        ...
        let pos = s.eft.pos.slice(1, s.ehlers_ft_options.pos_length + 1),
