...
module.exports = {
  ...
  onPeriod: function (s) {
    ema(s, 'trendline', s.trendline_options.trendline)
    var tl1 = []
    var tls = []
    var tll = []
    if (s.lookback[s.trendline_options.avgpoints + 2000]) {
      for (let i = 0; i < s.trendline_options.avgpoints + 1000; i++) { tl1.push(s.lookback[i].close) }
      for (let i = 0; i < s.trendline_options.lastpoints; i++) { tls.push(s.lookback[i].close) }
      for (let i = 0; i < s.trendline_options.avgpoints; i++) { tll.push(s.lookback[i].close) }

      var chart = tl1

      var growth = trend(chart, {
        lastPoints: s.trendline_options.lastpoints,
        avgPoints: s.trendline_options.avgpoints,
        avgMinimum: 0,
        reversed: true
      })
      var growth2 = trend(chart, {
        lastPoints: s.trendline_options.lastpoints2,
        avgPoints: s.trendline_options.avgpoints2,
        avgMinimum: 0,
        reversed: true
      })
      ...
      s.trendline_options.markup_sell_pct = math.mean(s.pcts, s.pctl) * 100
      s.trendline_options.markdown_buy_pct = math.mean(s.pcts, s.pctl) * 100
  ...
  onReport: function (s) {
    ...
    // cols.push(' ')
    // cols.push(z(8, n(s.trendline_options.markup_sell_pct).format('0.00000000'), ' ')[s.accel === true ? 'green' : 'red'])
    // return cols
  },
