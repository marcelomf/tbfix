...
module.exports = {
  ...
  calculate: function (s) {
    if (s.dema_options.overbought_rsi) {
      // sync RSI display with overbought RSI periods
      s.dema_options.rsi_periods = s.dema_options.overbought_rsi_periods
      rsi(s, 'overbought_rsi', s.dema_options.overbought_rsi_periods)
      if (!s.in_preroll && s.period.overbought_rsi >= s.dema_options.overbought_rsi && !s.overbought) {
        s.overbought = true
        if (s.dema_options.mode === 'sim' && s.dema_options.verbose) console.log(('\noverbought at ' + s.period.overbought_rsi + ' RSI, preparing to sold\n').cyan)
      }
    }

    // compture DEMA
    ema(s, 'ema_short', s.dema_options.ema_short_period)
    ema(s, 'ema_long', s.dema_options.ema_long_period)
  ...
  onPeriod: function (s) {
    ...
    if (typeof s.period.dema_histogram === 'number' && typeof s.lookback[0].dema_histogram === 'number') {
      if (s.dema_options.noise_level_pct != 0 && (s.period.ema_short / s.lookback[0].ema_short * 100 < s.dema_options.noise_level_pct)) {
        s.signal = null
      } else if ((s.period.dema_histogram - s.dema_options.up_trend_threshold) > 0 && (s.lookback[0].dema_histogram - s.dema_options.up_trend_threshold) <= 0) {
        s.signal = 'buy'
      } else if ((s.period.dema_histogram + s.dema_options.down_trend_threshold) < 0 && (s.lookback[0].dema_histogram + s.dema_options.down_trend_threshold) >= 0) {
        s.signal = 'sell'
      } else {
        s.signal = null  // hold
      }
