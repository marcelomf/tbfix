...
module.exports = {
  ...
  calculate: function (s) {
    if (s.lookback[1]) {
      ...
      if (s.lookback[s.speed_options.baseline_periods + 1]) {
        ema(s, 'baseline', s.speed_options.baseline_periods, 'abs_speed')
      }
    }
  },
  ...
  onPeriod: function (s) {
    if (typeof s.period.baseline === 'number') {
      if (s.period.speed >= s.period.baseline * s.speed_options.trigger_factor) {
        if (s.trend !== 'up') {
      ...
      else if (s.period.speed <= s.period.baseline * s.speed_options.trigger_factor * -1) {
        if (s.trend !== 'down') {
          s.acted_on_trend = false
        }
        s.trend = 'down'
