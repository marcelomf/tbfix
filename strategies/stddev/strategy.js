...
module.exports = {
  ...
  onPeriod: function (s) {
    ema(s, 'stddev', s.stddev_options.stddev)
    var tl0 = []
    var tl1 = []
    if (s.lookback[s.stddev_options.min_periods]) {
      for (let i = 0; i < s.stddev_options.trendtrades_1; i++) { tl0.push(s.lookback[i].close) }
      for (let i = 0; i < s.stddev_options.trendtrades_2; i++) { tl1.push(s.lookback[i].close) }
