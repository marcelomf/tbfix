...
module.exports = {
  ...
  calculate: function (s) {
    rsi(s, 'rsi_idx', s.rsi_options.rsi_periods)
  },

  onPeriod: function (s) {
    if (s.in_preroll) return
    if (typeof s.period.rsi_idx === 'number') {
      if (s.trend === undefined && s.period.rsi_idx <= s.rsi_options.oversold_rsi) {
        s.rsi_low = s.period.rsi_idx
        s.trend = 'oversold'
      }
      if (s.trend === 'oversold') {
        s.rsi_low = Math.min(s.rsi_low, s.period.rsi_idx)
        if (s.period.rsi_idx >= s.rsi_low + s.rsi_options.rsi_recover) {
          s.trend = 'long'
          s.signal = 'buy'
          s.rsi_high = s.period.rsi_idx
        }
      }
      if (s.trend !== 'oversold' && s.trend !== 'long' && s.period.rsi_idx >= s.rsi_options.overbought_rsi) {
        s.rsi_high = s.period.rsi_idx
        s.trend = 'long'
      }
      if (s.trend === 'long') {
        s.rsi_high = Math.max(s.rsi_high, s.period.rsi_idx)
        if (s.period.rsi_idx <= s.rsi_high / s.rsi_options.rsi_divisor) {
          s.trend = 'short'
          s.signal = 'sell'
        }
      }
      if (s.trend === 'long' && s.period.rsi_idx >= s.rsi_options.overbought_rsi) {
        s.rsi_high = s.period.rsi_idx
        s.trend = 'overbought'
      }
      if (s.trend === 'overbought') {
        s.rsi_high = Math.max(s.rsi_high, s.period.rsi_idx)
        if (s.period.rsi_idx <= s.rsi_high - s.rsi_options.rsi_drop) {
          s.trend = 'short'
          s.signal = 'sell'
        }
      }
  ...
  onReport: function (s) {
    ...
    //   var color = 'grey'
    //   if (s.period.rsi <= s.rsi_options.oversold_rsi) {
    //     color = 'green'
    //   }
    //   if (s.period.rsi >= s.rsi_options.overbought_rsi) {
    //     color = 'red'
    //   }
