...
module.exports = function (program, conf) {
  program
    ...
    .action(async function (selectorString, cmd) {
      selector = selector(selectorString || conf.selector)
      var exchange = require(`../exchanges/${selector.exchange_id}/exchange`)(conf)
      if (!exchange) {
        console.error('cannot backfill ' + selector.normalized + ': exchange not implemented')
        process.exit(1)
      }

      var collectionInstance = collection(conf)
      var tradesCollection = collectionInstance.getTrades()
      var resume_markers = collectionInstance.getResumeMarkers()
      ...
      let resultsMakers = await resume_markers.find({selector: selector.normalized}).toArray()
      markers = resultsMakers.sort(function (a, b) {
        if (mode === 'backward') {
      ...
      async function getNext () {
        ...
        // console.log(opts)
        if(opts.from && selector.normalized && selector.normalized.search('USD') >= 0) { // busca no minimo de 4 em 4 minutos
          // console.log('ATUAL')
          // console.log(moment(opts.from).format("YYYY-MM-DD HH:mm:ss"))
          if(beforeFrom) {
            let duration = moment.duration(moment(opts.from).diff(beforeFrom))
            if(duration.asMinutes() < 4) {
              // console.log('CORRIGIDO')
              // console.log(moment(beforeFrom).add(4, 'minutes').format("YYYY-MM-DD HH:mm:ss"))
              // console.log(moment(beforeFrom).add(4, 'minutes').toDate().getTime())
              opts.from = moment(beforeFrom).add(4, 'minutes').toDate().getTime()
            }
          }
          beforeFrom = opts.from
        }
        // console.log(moment(opts.from).format("YYYY-MM-DD HH:mm:ss"))
      ...
      function runTasks (trades) {
        Promise.all(trades.map((trade) => saveTrade(trade))).then(async function(/*results*/){
          var oldest_time = marker.oldest_time
          var newest_time = marker.newest_time
          markers.forEach(async function (other_marker) {
          ...
          await resume_markers.save(marker)
          setupNext()
        }).catch(function(err){
          if (err) {
            console.error(err)
