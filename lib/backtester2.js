let _ = require('lodash')
  ...
  , selectorInstance = require('./selector')
  , z = require('zero-fill')
  , n = require('numbro')
...
let processOutput = function (output, taskStrategyName, pheno) {
  ...
  if (typeof (simulationResults) === 'object' && typeof simulationResults.simresults !== typeof undefined) {
    ...
    params.selector = selectorInstance(selector)
  ...
  let roi
  if (params.currency_capital == 0.0 || !params.currency_capital) {
...
let runUpdate = function (days, selector) {
  let zenbot_cmd = process.platform === 'win32' ? 'tbfix.bat' : './tbfix.sh'
