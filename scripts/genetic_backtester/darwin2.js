...
let Backtester = require('../../lib/backtester2')
...
function allStrategyNames() {
  let pathName = path.resolve(__dirname, '..', '..', 'strategies')
...
function simulateGeneration(generateLaunchFile) {
  ...
  let tasks = selectedStrategies.map(v => pools[v]['pool'].population().map(phenotype => {
    return cb => {
      ...
      //command.commandString = command.commandString+" --reverse"
...
for (var i = 0; i < selectedStrategies.length; i++) {
  ...
  let strategyData = require(path.resolve(__dirname, `../../strategies/${v}/strategy`))
