...
for(let i = 0; i < stts.length; i++) {
	if(!validateResults.find(e => e == "("+stts[i]+"5m.buyAND("+stts[i]+"15m.sellOR"+stts[i]+"20m.sellOR"+stts[i]+"25m.sellOR"+stts[i]+"30m.sell))")) {
		results.push({
			strategies: "candle.5m,"+stts[i]+".5m,"+stts[i]+".15m,"+stts[i]+".20m,"+stts[i]+".25m,"+stts[i]+".30m",
			strategies_action_buy: "(candle5m.buyAND"+stts[i]+"5m.buyAND("+stts[i]+"15m.sellOR"+stts[i]+"20m.sellOR"+stts[i]+"25m.sellOR"+stts[i]+"30m.sell))",
			strategies_action_sell: "(candle5m.sellAND"+stts[i]+"5m.sellAND("+stts[i]+"15m.buyOR"+stts[i]+"20m.buyOR"+stts[i]+"25m.buyOR"+stts[i]+"30m.buy))"
		})
	...
	if(!validateResults.find(e => e == "("+stts[i]+"5m.buyAND"+stts[i]+"10m.buy)")) {
		results.push({
			strategies: stts[i]+".5m,"+stts[i]+".10m,candle.5m,candle.10m",
			strategies_action_buy: "("+stts[i]+"5m.buyAND"+stts[i]+"10m.buyAND(candle5m.buyORcandle10m.buy))",
			strategies_action_sell: "("+stts[i]+"5m.sellOR"+stts[i]+"10m.sellAND(candle5m.sellORcandle10m.sell))",
		})
		validateResults.push("("+stts[i]+"5m.buyAND"+stts[i]+"10m.buy)")
	}
	if(!validateResults.find(e => e == "("+stts[i]+"15m.buyAND"+stts[i]+"30m.buy)")) {
		results.push({
			strategies: stts[i]+".15m,"+stts[i]+".30m,candle.15m,candle.30m",
			strategies_action_buy: "("+stts[i]+"15m.buyAND"+stts[i]+"30m.buyAND(candle15m.buyORcandle30m.buy))",
			strategies_action_sell: "("+stts[i]+"15m.sellOR"+stts[i]+"30m.sellAND(candle15m.sellORcandle30m.sell))",
		})
	...
	for(let x = 0; x < stts.length; x++) {
		...
		if(!validateResults.find(e => e == "("+stts[i]+"5m.buyAND"+stts[x]+"5m.buy)")) {
			results.push({
				strategies: stts[i]+".5m,"+stts[x]+".5m,candle.5m",
				strategies_action_buy: "("+stts[i]+"5m.buyAND"+stts[x]+"5m.buyANDcandle5m.buy)",
				strategies_action_sell: "(("+stts[i]+"5m.sellOR"+stts[x]+"5m.sell)ANDcandle5m.sell)"
			})
			validateResults.push("("+stts[i]+"5m.buyAND"+stts[x]+"5m.buy)")
		}
		if(!validateResults.find(e => e == "("+stts[i]+"15m.buyAND"+stts[x]+"15m.buy)")) {
			results.push({
				strategies: stts[i]+".15m,"+stts[x]+".15m,candle.15m",
				strategies_action_buy: "("+stts[i]+"15m.buyAND"+stts[x]+"15m.buyANDcandle15m.buy)",
				strategies_action_sell: "(("+stts[i]+"15m.sellOR"+stts[x]+"15m.sell)ANDcandle15m.sell)"
			})
		...
		if(!validateResults.find(e => e == "("+stts[i]+"30m.buyAND"+stts[x]+"30m.buy)")) {
			results.push({
				strategies: stts[i]+".30m,"+stts[x]+".30m,candle.30m",
				strategies_action_buy: "("+stts[i]+"30m.buyAND"+stts[x]+"30m.buyANDcandle30m.buy)",
				strategies_action_sell: "(("+stts[i]+"30m.sellOR"+stts[x]+"30m.sell)ANDcandle30m.sell)"
			})
